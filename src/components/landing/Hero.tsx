  ...
  return (
    <section
      className={`relative min-h-[100svh] w-full overflow-hidden transition-colors duration-700 ${
        isRevenue ? "bg-[#a1a1a3] text-[#111214]" : "bg-[#d4d4d4] text-[#111214]"
      }`}
    >
      <BoomerangVideoBg
        src={BG_VIDEO}
      ...
      <div
        className={`absolute -right-24 top-20 h-80 w-80 rounded-full blur-3xl transition-colors duration-700 ${
          isRevenue ? "bg-[#6b6b6b]/20" : "bg-[#c5f04a]/25"
        }`}
      />
      <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-[#ffffff] to-transparent" />
      ...
      <nav className="absolute inset-x-0 top-0 z-30 flex items-center justify-between px-4 py-4 sm:px-6 sm:py-6 md:px-10">
        ...
        <div className="hidden items-center gap-1 rounded-full border border-white/70 bg-white/78 py-1 pl-6 pr-1 shadow-sm backdrop-blur-xl lg:flex">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="px-3 py-2 text-sm font-medium text-[#26272b] transition-colors hover:text-[#08090a]"
            >
              {link.label}
            </Link>
          ))}
          <a
            ...
            className={`ml-2 rounded-full px-5 py-2.5 text-sm font-semibold text-white transition-all hover:-translate-y-0.5 ${
              isRevenue ? "bg-[#4a4b4f] hover:bg-[#4a4b4f]" : "bg-[#0a0a0a] hover:bg-[#1c1d20]"
            }`}
          >
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          <a
            ...
            target={primaryIsExternal ? "_blank" : undefined}
            rel={primaryIsExternal ? "noopener noreferrer" : undefined}
            className="hidden items-center gap-2 text-sm font-semibold text-[#1c1d20] transition-opacity hover:opacity-70 sm:flex"
          >
          ...
          <button
            ...
            className="relative flex h-10 w-10 items-center justify-center rounded-full border border-white/70 bg-white/80 text-[#0a0a0a] backdrop-blur-md transition-colors hover:bg-white lg:hidden"
      ...
      {menuOpen && (
        <>
          <div className="fixed inset-0 z-20 bg-[#0a0a0a]/40 backdrop-blur-sm lg:hidden" onClick={() => setMenuOpen(false)} />
          <div
            className="fixed bottom-0 right-0 top-0 z-20 w-[86%] max-w-sm bg-[#ffffff]/97 shadow-2xl backdrop-blur-xl lg:hidden"
            role="dialog"
            aria-modal="true"
            ...
            <div className="flex h-full flex-col px-8 pb-8 pt-24">
              <div className="flex flex-col gap-1">
                {NAV_LINKS.map((link, index) => (
                  <Link
                    ...
                    className="translate-x-0 border-b border-[#0a0a0a]/10 py-4 font-display text-3xl text-[#0a0a0a] opacity-100 transition-all duration-500"
              ...
              <div className="mt-8 grid translate-x-0 gap-3 opacity-100 transition-all duration-500 delay-300">
                ...
                <a
                  ...
                  onClick={() => setMenuOpen(false)}
                  className="inline-flex items-center justify-center gap-2 rounded-full border border-[#0a0a0a]/15 bg-white px-5 py-3 text-sm font-semibold text-[#0a0a0a]"
                >
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-[1440px] flex-col px-5 pb-20 pt-24 sm:px-8 sm:pt-28 lg:px-12 lg:pb-24">
        <div className="flex w-full flex-col gap-2 rounded-[1.4rem] border border-white/75 bg-white/70 p-2 shadow-[0_16px_60px_rgba(10, 10, 10,0.08)] backdrop-blur-xl sm:w-fit sm:flex-row sm:items-center sm:rounded-full">
          <span className="px-3 py-1 text-center font-mono text-[9px] font-medium uppercase tracking-[0.17em] text-[#4a4b4f] sm:text-left">
            ¿Qué buscas?
          </span>
          <div
            className="grid grid-cols-2 gap-1 rounded-2xl bg-[#ebebeb]/85 p-1 sm:rounded-full"
            role="group"
          ...
          >
            <button
              ...
              className={`flex min-h-10 items-center justify-center gap-2 rounded-xl px-3 text-xs font-semibold transition-all duration-300 sm:rounded-full sm:px-5 sm:text-sm ${
                isRevenue
                  ? "bg-[#4a4b4f] text-white shadow-[0_8px_24px_rgba(185,71,45,0.24)]"
                  : "text-[#6b6b6b] hover:bg-white/75 hover:text-[#111214]"
              }`}
            >
            ...
            <button
              ...
              className={`flex min-h-10 items-center justify-center gap-2 rounded-xl px-3 text-xs font-semibold transition-all duration-300 sm:rounded-full sm:px-5 sm:text-sm ${
                !isRevenue
                  ? "bg-[#0a0a0a] text-white shadow-[0_8px_24px_rgba(10, 10, 10,0.2)]"
                  : "text-[#6b6b6b] hover:bg-white/75 hover:text-[#111214]"
              }`}
            >
        ...
        >
          <div className="max-w-4xl">
            <div
              className={`mb-5 inline-flex items-center gap-2 rounded-full border bg-white/72 px-4 py-2 font-mono text-[9px] font-medium uppercase tracking-[0.16em] backdrop-blur ${
                isRevenue
                  ? "border-[#4a4b4f]/15 text-[#4a4b4f]"
                  : "border-[#1c1d20]/15 text-[#1c1d20]"
              }`}
            >
            ...
            <h1
              className={`max-w-5xl font-display text-[clamp(3.15rem,6.8vw,7.1rem)] font-normal leading-[0.86] tracking-[-0.045em] ${
                isRevenue ? "text-[#111214]" : "text-[#0a0a0a]"
              }`}
            >
              {content.title}
            </h1>
            <p
              className={`mt-6 max-w-2xl text-base leading-relaxed sm:text-lg md:text-xl ${
                isRevenue ? "text-[#26272b]" : "text-[#26272b]"
              }`}
            >
              {content.body}
            </p>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
              <a
                ...
                target={primaryIsExternal ? "_blank" : undefined}
                rel={primaryIsExternal ? "noopener noreferrer" : undefined}
                className={`inline-flex min-h-12 items-center justify-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-white shadow-[0_18px_42px_rgba(10, 10, 10,0.18)] transition-all duration-300 hover:-translate-y-0.5 ${
                  isRevenue ? "bg-[#4a4b4f] hover:bg-[#4a4b4f]" : "bg-[#0a0a0a] hover:bg-[#1c1d20]"
                }`}
              >
                {content.primaryLabel}
              ...
              <a
                ...
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[#0a0a0a]/15 bg-white/75 px-6 py-3 text-sm font-semibold text-[#0a0a0a] backdrop-blur transition-all duration-300 hover:-translate-y-0.5 hover:bg-white"
              >
          ...
          <div className="hidden lg:block">
            <div className="rotate-[1.5deg] overflow-hidden rounded-[1.7rem] border border-white/55 bg-[#ffffff]/88 p-3 shadow-[0_38px_110px_rgba(10, 10, 10,0.2)] backdrop-blur-xl transition-transform duration-500 hover:rotate-0 hover:scale-[1.01]">
              <div className="flex items-center justify-between px-2 pb-3 pt-1">
                <div className="font-mono text-[9px] uppercase tracking-[0.16em] text-[#4a4b4f]">
                  {content.visualLabel}
                </div>
                <ArrowRight className={`size-4 -rotate-45 ${isRevenue ? "text-[#4a4b4f]" : "text-[#0a0a0a]"}`} />
              </div>
              <div className="relative aspect-[16/11] overflow-hidden rounded-[1.1rem] bg-[#ebebeb]">
                <Image
                ...
                />
                <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-[#0a0a0a]/45 to-transparent" />
                <div className="absolute bottom-4 left-4 right-4 flex flex-wrap gap-1.5">
                  {content.visualTags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-full border border-white/25 bg-[#0a0a0a]/78 px-3 py-1.5 font-mono text-[8px] uppercase tracking-[0.12em] text-white backdrop-blur"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="grid grid-cols-[1fr_auto] items-end gap-6 px-2 pb-2 pt-5">
                <h2 className="max-w-[13ch] font-display text-3xl leading-[0.92] text-[#0a0a0a]">
                  {content.visualTitle}
                </h2>
                <div className="text-right">
                  <div className={`font-display text-4xl leading-none ${isRevenue ? "text-[#4a4b4f]" : "text-[#0a0a0a]"}`}>
                    {content.visualMetric}
                  </div>
                  <div className="mt-1 max-w-28 font-mono text-[8px] uppercase leading-4 tracking-[0.12em] text-[#4a4b4f]">
          ...
          <div className="grid gap-px overflow-hidden rounded-2xl border border-[#0a0a0a]/10 bg-[#0a0a0a]/10 sm:grid-cols-3 lg:col-span-2">
            {content.steps.map(([number, title, description]) => (
              <div key={number} className="grid grid-cols-[auto_1fr] gap-4 bg-white/62 p-5 backdrop-blur-md sm:block">
                <span className={`font-mono text-[9px] tracking-[0.18em] ${isRevenue ? "text-[#4a4b4f]" : "text-[#0a0a0a]"}`}>
                  {number}
                </span>
                <div>
                  <h3 className="font-display text-xl text-[#0a0a0a] sm:mt-5 sm:text-2xl">{title}</h3>
                  <p className="mt-1 text-xs leading-5 text-[#4a4b4f] sm:mt-2 sm:text-sm">{description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <button
        ...
        className="absolute bottom-5 right-6 z-10 hidden items-center gap-2 rounded-full bg-[#0a0a0a]/72 px-4 py-2 text-sm font-semibold text-white backdrop-blur-md transition-colors hover:bg-[#0a0a0a] md:right-10 xl:flex"
