...
const FOUNDER_MESSAGE =
  "Hola, quiero reservar un cupo de los primeros 5 negocios para el Desk Cohorte ($390 setup + $79/mes).";
...
const PRICING = [
  {
    id: "starter",
    name: "Desk Cohorte",
    price: "$390 setup + $79/mes",
    bestFor: "Los primeros 5 negocios de Caracas. Mismo sistema, precio de cohorte.",
    cta: "Reservar cupo de cohorte",
    features: [
      "Todo el Desk, sin recortes",
      "A cambio: feedback semanal",
      "Permiso para documentar el proceso",
      "Metricas operativas anonimizadas",
      "Testimonio solo si quedas conforme",
    ],
  },
  {
    id: "growth",
    name: "Desk",
    price: "$690 setup + $129/mes",
    bestFor: "Negocios que quieren calificar leads, agendar mejor y tener mas control.",
    cta: "Instalar el Desk",
    recommended: true,
    features: [
      "Inbox comercial para tu equipo",
      "Respuestas sugeridas con IA, aprobadas por vos",
      "Flujo de calificacion y seguimientos",
      "Mini landing/cotizador",
      "Cada chat marcado: cita, cotizacion o descarte",
      "Tu linea base medida a los 30 dias",
    ],
  },
  {
    id: "premium",
    name: "Desk Empresa",
    price: "$1,490 setup + $290/mes",
    bestFor: "Varias sedes, equipo comercial o volumen de anuncios.",
    cta: "Aplicar a Empresa",
    features: [
      "Todo el Desk",
      "Multiples sedes o numeros",
      "Funnel completo desde Instagram/anuncios",
      "Hermes Agent para orquestacion, borradores y resumenes",
      "Segmentacion de leads y reporte semanal",
      "Habilita el Sprint a Resultado: piso + cobro por cita atribuida",
    ],
  },
];
...
function PrimaryButton({
  ...
  return (
    <a
      ...
      className={`inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#111214] px-5 py-3 text-sm font-semibold text-white shadow-[0_18px_50px_rgba(23,51,34,0.18)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#17181a] ${className}`}
    >
      {children}
...
function SecondaryButton({
  ...
  return (
    <a
      ...
      target={href.startsWith("http") ? "_blank" : undefined}
      rel={href.startsWith("http") ? "noopener noreferrer" : undefined}
      className={`inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[#111214]/15 bg-white px-5 py-3 text-sm font-semibold text-[#111214] transition-all duration-300 hover:-translate-y-0.5 hover:border-[#111214]/30 hover:bg-[#f3f3f3] ${className}`}
    >
      {children}
    </a>
  );
...
function HeroMockup() {
  ...
  return (
    <div className="relative mx-auto w-full max-w-lg">
      <div className="absolute -left-4 top-10 hidden h-36 w-36 rounded-full border border-[#111214]/10 md:block" />
      <div className="relative overflow-hidden rounded-[28px] border border-[#111214]/12 bg-[#ffffff] p-3 shadow-[0_28px_90px_rgba(23,51,34,0.16)]">
        <div className="rounded-[22px] bg-[#111214] p-4 text-white">
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#c5f04a] text-[#111214]">
          ...
          <div className="space-y-3 rounded-2xl bg-[#f3f3f3] p-4 text-[#111214]">
            ...
            <div className="ml-auto max-w-[86%] rounded-2xl rounded-br-sm bg-[#d9f77e] px-4 py-3 text-sm shadow-sm">
            ...
            <div className="ml-auto max-w-[88%] rounded-2xl rounded-br-sm bg-[#d9f77e] px-4 py-3 text-sm shadow-sm">
            ...
            <div key={item} className="flex items-center gap-3">
              <div
                className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                  index < 4 ? "bg-[#111214] text-white" : "bg-[#6b6b6b] text-white"
                }`}
              >
                {index + 1}
              </div>
              <div className="h-2 flex-1 rounded-full bg-[#f3f3f3]">
                <div
                  className={`h-full rounded-full ${index < 4 ? "bg-[#c5f04a]" : "bg-[#6b6b6b]"}`}
                  style={{ width: `${44 + index * 13}%` }}
                />
              </div>
              <span className="w-28 text-xs font-semibold text-[#111214]">{item}</span>
            </div>
          ))}
        </div>
      </div>
...
function HeroBackground() {
  ...
  return (
    <div aria-hidden="true" className="absolute inset-0 overflow-hidden">
      <div className="absolute inset-0 bg-[linear-gradient(116deg,#ffffff_0%,#f3f3f3_42%,#f3f3f3_100%)]" />
      <div
      ...
      {signalBars.map(([top, right, width]) => (
        <div
          key={`${top}-${right}`}
          className={`absolute ${top} ${right} hidden ${width} rounded-full border border-[#111214]/10 bg-white/44 px-3 py-2 shadow-sm backdrop-blur-sm md:block`}
        >
          <div className="h-1.5 rounded-full bg-[#111214]/18" />
        </div>
      ))}
      ...
      {messageGhosts.map(([topOrBottom, side, width, rotate]) => (
        ...
        >
          <div className="h-1.5 w-2/3 rounded-full bg-[#111214]/13" />
          <div className="mt-2 h-1.5 w-full rounded-full bg-[#c5f04a]/18" />
        </div>
      ))}

      <div className="absolute bottom-0 left-0 right-0 h-px bg-[#111214]/10" />
    </div>
  );
}
            ...
            {[
              ...
              <div key={label as string} className="grid gap-2">
                <div className="flex justify-between items-center text-xs font-semibold uppercase tracking-[0.16em]">
                  ...
                  <span className="text-[#c5f04a] font-mono text-sm font-bold">{current as number}</span>
                </div>
                <input
                  type="range"
                  min={min as number}
                  ...
                  className="w-full h-1 bg-white/10 rounded-lg appearance-none cursor-pointer accent-[#c5f04a] hover:bg-white/20 transition-all"
                />
              </div>
            ))}
            <div className="md:col-span-3 mt-6 grid gap-4 rounded-2xl bg-[#f3f3f3] p-5 text-[#111214] md:grid-cols-[1fr_auto] md:items-center">
              <div>
                <div className="text-sm font-semibold text-[#4a4b4f]">Estimacion simple</div>
                <p className="mt-1 text-base sm:text-lg font-semibold">
                  ...
                  <span className="text-[#4a4b4f]">${result.lostRevenue.toLocaleString("en-US")}</span> sobre la mesa.
                </p>
              </div>
...
function DemoSection() {
  ...
  return (
    <section id="demo" className="bg-[#f3f3f3] text-[#111214] scroll-mt-24">
      <div className="mx-auto grid max-w-6xl gap-10 px-6 py-20 md:grid-cols-[0.86fr_1.14fr] md:px-10 md:py-28">
        <div>
          ...
          <div className="mt-8 inline-flex rounded-full border border-[#111214]/12 bg-white p-1">
            {Object.entries(DEMOS).map(([key, item]) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${
                  active === key ? "bg-[#111214] text-white" : "text-[#6b6b6b] hover:text-[#111214]"
                }`}
              >
                {item.label}
              </button>
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_220px]">
          <div className="rounded-[28px] border border-[#111214]/12 bg-white p-4 shadow-[0_20px_60px_rgba(23,51,34,0.08)]">
            <div className="mb-4 flex items-center justify-between border-b border-[#111214]/8 pb-4">
              <div>
                <div className="text-sm font-semibold">{demo.title}</div>
                <div className="text-xs text-[#6b6b6b]">Fuente: {demo.source}</div>
              </div>
              <div className="rounded-full bg-[#d9f77e] px-3 py-1 text-xs font-semibold text-[#17181a]">
                {demo.status}
              </div>
            </div>
              ...
              {demo.messages.map(([role, text], index) => (
                <div
                  ...
                  className={`max-w-[86%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                    role === "client"
                      ? "rounded-bl-sm bg-[#ebebeb] text-[#111214]"
                      : "ml-auto rounded-br-sm bg-[#d9f77e] text-[#111214]"
                  }`}
                >
                  {text}
                </div>
          ...
          <div className="rounded-[24px] border border-[#111214]/12 bg-[#111214] p-5 text-white">
            ...
            <div className="mt-5 rounded-xl bg-[#6b6b6b] px-3 py-3 text-sm font-semibold text-white">
...
function AuditForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-[28px] border border-[#111214]/12 bg-white p-5 shadow-[0_20px_70px_rgba(23,51,34,0.1)] md:p-6">
      <div className="grid gap-4 sm:grid-cols-2">
        {[
          ...
          <label key={label} className="grid gap-2">
            <span className="text-xs font-semibold uppercase tracking-[0.14em] text-[#4a4b4f]">
              {label}
            </span>
            <input
              required
              type={type}
              className="h-12 rounded-xl border border-[#111214]/12 bg-[#f3f3f3] px-4 text-sm font-medium text-[#111214] outline-none transition focus:border-[#c5f04a] focus:ring-2 focus:ring-[#c5f04a]/25"
            />
          </label>
        ))}
      </div>
      ...
      {submitted ? (
        <div className="mt-5 rounded-2xl bg-[#d9f77e] p-4 text-sm font-medium leading-relaxed text-[#111214]">
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row">
        <button
          type="submit"
          className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-[#111214] px-5 py-3 text-sm font-semibold text-white transition-all hover:-translate-y-0.5 hover:bg-[#17181a]"
        >
          Enviar auditoria
...
export default function WhatsAppRevenuePage() {
  ...
  return (
    <main className="relative bg-[#f3f3f3] text-[#111214] selection:bg-[#c5f04a]/45 selection:text-[#111214]">
      <header className="absolute inset-x-0 top-0 z-30">
        <nav className="mx-auto flex h-20 max-w-6xl items-center justify-between px-4 sm:px-6 md:px-10">
          <Link href="/" aria-label="Releva" className="text-[#111214]">
            <RelevaLogo variant="lockup-bare" className="h-8 w-auto" />
          </Link>

          <div className="hidden items-center gap-1 rounded-full border border-[#111214]/10 bg-white/78 py-1 pl-5 pr-1 shadow-sm backdrop-blur-md lg:flex">
            {NAV_LINKS.map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="px-3 py-2 text-sm font-medium text-[#6b6b6b] transition-colors hover:text-[#111214]"
              >
                {item.label}
              </a>
            ))}
            <a
              href="#demo"
              className="ml-2 inline-flex items-center gap-2 rounded-full bg-[#f3f3f3] px-4 py-2.5 text-sm font-semibold text-[#111214] transition-colors hover:bg-[#d9f77e]"
            >
              Ver demo
            </a>
            <a
              href="#auditoria"
              className="inline-flex items-center gap-2 rounded-full bg-[#111214] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#17181a]"
            >
              Agendar auditoria
            </a>
          </div>
          ...
          <a
            ...
            className="inline-flex items-center gap-2 rounded-full bg-[#111214] px-4 py-2.5 text-sm font-semibold text-white lg:hidden"
          >
            Auditoria
          </a>
        </nav>
      ...
      <section className="relative overflow-hidden pt-24 md:pt-24">
        ...
        <div className="relative mx-auto grid gap-10 px-6 pb-14 md:min-h-[760px] md:grid-cols-[1.1fr_0.9fr] md:items-center md:px-10 lg:max-w-6xl">
          <div>
            <h1 className="max-w-4xl text-[3.35rem] font-normal leading-[0.95] tracking-tight text-[#111214] sm:text-6xl md:text-7xl lg:text-[5rem]">
              Convierte tus mensajes de WhatsApp en clientes.
            </h1>
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-[#26272b] md:text-xl">
            ...
            <p className="mt-4 max-w-xl text-sm leading-relaxed text-[#8a8a8a] md:text-base">
            ...
            <div className="mt-6 flex flex-wrap gap-x-5 gap-y-2 text-sm font-medium text-[#8a8a8a]">
      ...
      <section className="bg-[#ffffff] text-[#111214]">
          ...
          <div className="mt-10 grid gap-3 md:grid-cols-4">
            {PAINS.map((item) => {
              ...
              return (
                <article
                  key={item.title}
                  className="rounded-2xl border border-[#111214]/10 bg-[#f3f3f3] p-5 transition-all duration-300 hover:-translate-y-1 hover:bg-white hover:shadow-[0_20px_55px_rgba(23,51,34,0.08)]"
                >
                  <Icon className="h-5 w-5 text-[#4a4b4f]" />
                  <h3 className="mt-5 text-lg font-semibold">{item.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-[#6b6b6b]">{item.copy}</p>
                </article>
              );
            })}
          </div>
      ...
      <section id="sistema" className="bg-[#f3f3f3] text-[#111214] scroll-mt-24">
          ...
          <div className="grid gap-10 md:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="grid gap-3">
              {SYSTEM_STEPS.map((step, index) => (
                <div
                  key={step}
                  className="grid grid-cols-[44px_1fr_auto] items-center gap-4 rounded-2xl border border-[#111214]/10 bg-white px-4 py-4"
                >
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#111214] text-sm font-bold text-white">
                    {index + 1}
                  </div>
                  <span className="text-base font-semibold">{step}</span>
                  <ChevronRight className="h-4 w-4 text-[#c5f04a]" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-16">
            ...
            <div className="mt-8 grid gap-3 md:grid-cols-3">
              {FEATURES.map((feature) => {
                ...
                return (
                  <article key={feature.title} className="rounded-2xl border border-[#111214]/10 bg-white p-5">
                    <Icon className="h-5 w-5 text-[#17181a]" />
                    <h3 className="mt-5 text-lg font-semibold">{feature.title}</h3>
                    <p className="mt-2 text-sm leading-relaxed text-[#6b6b6b]">{feature.copy}</p>
                  </article>
                );
              })}
            </div>
      ...
      <section className="bg-[#ffffff] text-[#111214]">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 py-20 md:grid-cols-[0.9fr_1.1fr] md:px-10 md:py-28">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {AUDIENCES.map((item) => (
              <div key={item} className="flex items-center gap-3 rounded-2xl border border-[#111214]/10 bg-[#f3f3f3] px-4 py-4 text-sm font-semibold">
                <CheckCircle2 className="h-4 w-4 shrink-0 text-[#c5f04a]" />
                {item}
              </div>
            ))}
          </div>
      ...
      <section className="bg-[#111214] text-white">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 py-20 md:grid-cols-[0.9fr_1.1fr] md:px-10 md:py-28">
          <div>
            ...
            <PrimaryButton href={whatsappUrl(AUDIT_MESSAGE)} className="mt-8 bg-white !text-[#111214] shadow-none hover:bg-[#d9f77e]">
      ...
      <section id="precios" className="bg-[#f3f3f3] text-[#111214] scroll-mt-24">
        <div className="mx-auto max-w-6xl px-6 py-20 md:px-10 md:py-28">
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div>
              <h2 className="text-4xl font-normal leading-[1] tracking-tight md:text-6xl">
                Un solo sistema. Tres precios segun tu tamano.
              </h2>
              <p className="mt-5 max-w-xl text-base leading-relaxed text-[#6b6b6b]">
                No hay version recortada: el precio de cohorte compra feedback y un caso documentado, no menos sistema. El cobro por resultado se cotiza aparte, recien cuando existe tu linea base.
              </p>
            </div>
            <button
              type="button"
              ...
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#6b6b6b] px-5 py-3 text-sm font-semibold text-white transition-all hover:-translate-y-0.5 hover:bg-[#4a4b4f] disabled:cursor-wait disabled:opacity-70"
            >
              {loadingPlan === "growth" ? "Abriendo pago..." : "Instalar el Desk"}
            ...
            {PRICING.map((plan) => (
              <article
                ...
                className={`relative flex h-full flex-col rounded-[26px] border p-6 shadow-sm ${
                  plan.recommended
                    ? "border-[#111214] bg-white shadow-[0_24px_70px_rgba(23,51,34,0.12)]"
                    : "border-[#111214]/10 bg-white/78"
                }`}
              >
                {plan.recommended ? (
                  <div className="absolute right-5 top-5 rounded-full bg-[#d9f77e] px-3 py-1 text-xs font-bold text-[#17181a]">
                    Mas recomendado
                  </div>
                ) : null}
                <h3 className="pr-28 text-2xl font-semibold">{plan.name}</h3>
                <div className="mt-4 text-3xl font-semibold tracking-tight text-[#111214]">
                  {plan.price}
                </div>
                <p className="mt-4 min-h-16 text-sm leading-relaxed text-[#6b6b6b]">{plan.bestFor}</p>
                <div className="mt-6 grid flex-1 gap-3">
                  {plan.features.map((feature) => (
                    <div key={feature} className="flex gap-2 text-sm text-[#6b6b6b]">
                      <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[#c5f04a]" />
                      <span>{feature}</span>
                    </div>
                  ))}
                ...
                <button
                  ...
                  className={`mt-8 inline-flex min-h-12 items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-semibold transition-all hover:-translate-y-0.5 disabled:cursor-wait disabled:opacity-70 ${
                    plan.recommended
                      ? "bg-[#111214] text-white hover:bg-[#17181a]"
                      : "border border-[#111214]/15 bg-[#f3f3f3] text-[#111214] hover:bg-[#d9f77e]"
                  }`}
                >
      ...
      <section className="bg-[#ffffff] text-[#111214]">
        <div className="mx-auto max-w-6xl px-6 py-20 md:px-10 md:py-24">
          <div className="grid overflow-hidden rounded-[32px] border border-[#111214]/10 bg-[#111214] text-white md:grid-cols-[1fr_0.8fr]">
            <div className="p-7 md:p-10">
              ...
              <div className="mt-8 text-3xl font-semibold text-[#d9f77e] md:text-4xl">
                Desk Cohorte por $390 setup + $79/mes
              </div>
              ...
              <div className="mt-6 grid gap-2">
                {HERMES_SERVICES.map((service) => (
                  <div key={service} className="flex gap-3 rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-sm text-white/78">
                    <Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-[#d9f77e]" />
                    <span>{service}</span>
                  </div>
                ))}
              </div>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <PrimaryButton href={whatsappUrl(FOUNDER_MESSAGE)} className="bg-white !text-[#111214] shadow-none hover:bg-[#d9f77e]">
                ...
                >
                  {loadingPlan === "starter" ? "Abriendo pago..." : "Empezar con el Desk Cohorte"}
                  <ArrowRight className="h-4 w-4" />
                </button>
              </div>
            </div>
            <div className="border-t border-white/10 bg-white/[0.05] p-7 md:border-l md:border-t-0 md:p-10">
              <div className="text-xs font-semibold uppercase tracking-[0.16em] text-white/45">
                Que medimos en tus primeros 30 dias
              </div>
              <div className="mt-6 space-y-4">
                {[
                  ["Chats con siguiente paso", "tu linea base"],
                  ["Citas agendadas", "contadas una por una"],
                  ["Tiempo hasta primera respuesta", "antes y despues"],
                ].map(([label, value]) => (
                  <div key={label} className="flex items-center justify-between gap-4 border-b border-white/10 pb-4">
                    <span className="text-sm text-white/68">{label}</span>
                    <span className="text-right text-sm font-semibold text-[#d9f77e]">{value}</span>
                  </div>
                ))}
              </div>
              <p className="mt-6 text-sm leading-relaxed text-white/55">
                Si a los 30 dias el porcentaje de chats con siguiente paso no sube, no cobras el
                mes 2 y te quedas con el sistema.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="bg-[#f3f3f3] text-[#111214]">
        <div className="mx-auto max-w-6xl px-6 py-20 md:px-10 md:py-28">
          <div className="grid gap-10 md:grid-cols-[0.85fr_1.15fr]">
            ...
            <div className="grid gap-3">
              {PROCESS.map(([day, title, copy]) => (
                <div key={day} className="grid gap-3 rounded-2xl border border-[#111214]/10 bg-white p-5 md:grid-cols-[92px_1fr]">
                  <div className="text-sm font-bold uppercase tracking-[0.14em] text-[#4a4b4f]">{day}</div>
                  <div>
                    <h3 className="text-xl font-semibold">{title}</h3>
                    <p className="mt-2 text-sm leading-relaxed text-[#6b6b6b]">{copy}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="bg-[#ffffff] text-[#111214]">
        <div className="mx-auto max-w-6xl px-6 py-20 md:px-10 md:py-28">
          <div className="grid gap-10 md:grid-cols-[0.9fr_1.1fr] md:items-end">
            ...
            <div className="rounded-[26px] border border-[#111214]/10 bg-[#f3f3f3] p-6">
              <ShieldCheck className="h-6 w-6 text-[#c5f04a]" />
          ...
          <div className="mt-10 grid gap-3 md:grid-cols-3">
            {CAPABILITIES.map((item) => (
              <div key={item} className="flex items-center gap-3 rounded-2xl border border-[#111214]/10 bg-white px-4 py-4 text-sm font-semibold">
                <Sparkles className="h-4 w-4 text-[#4a4b4f]" />
                {item}
              </div>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-6xl px-6 py-20 md:px-10 md:py-28">
          ...
          <div className="mt-10 grid gap-3 md:grid-cols-2">
            {FAQS.map(([question, answer]) => (
              <details key={question} className="group rounded-2xl border border-[#111214]/10 bg-white p-5">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-lg font-semibold">
                  {question}
                  <ChevronRight className="h-4 w-4 shrink-0 transition-transform group-open:rotate-90" />
                </summary>
                <p className="mt-4 text-sm leading-relaxed text-[#6b6b6b]">{answer}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <section id="auditoria" className="bg-[#ffffff] text-[#111214] scroll-mt-24">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 py-20 md:grid-cols-[0.9fr_1.1fr] md:px-10 md:py-28">
          <div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[#111214]/10 bg-[#ffffff]/95 p-3 shadow-[0_-18px_50px_rgba(23,51,34,0.1)] backdrop-blur md:hidden">
        <a
          href={whatsappUrl(AUDIT_MESSAGE)}
          target="_blank"
          rel="noopener noreferrer"
          className="flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#111214] px-5 py-3 text-sm font-semibold text-white"
        >
          Auditoria gratis
