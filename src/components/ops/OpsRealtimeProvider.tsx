...
export default function OpsRealtimeProvider({ children }: { children: ReactNode }) {
  ...
  const label = {
    ...
    polling: "Actualización cada 15 s",
    offline: "Sin conexión",
  }[realtime.status];
  const dotClass = realtime.status === "connected"
    ? "bg-[#a8d51f]"
    : realtime.status === "reconnecting" || realtime.status === "connecting" || realtime.status === "polling"
      ? "bg-[#b58a1b]"
      : "bg-[#9aa6b4]";
  ...
  return (
    <OpsRealtimeContext.Provider value={realtime}>
      ...
      <div
        ...
        role="status"
        aria-live="polite"
        aria-label={realtime.status === "polling" ? label : `Realtime: ${label}. ${realtime.onlineOperators.length} operadores online.`}
      >
        <span className={`size-2 rounded-full ${dotClass}`} aria-hidden="true" />
        <span>{label}</span>
        {realtime.status !== "polling" && <><span className="text-[#9aa6b4]">·</span><span>{realtime.onlineOperators.length} online</span></>}
      </div>
