...
export default function VariantB() {
  ...
  return (
    <div className="allok">
      ...
      <section id="producto" className="mx-auto max-w-[1240px] px-5 py-24 sm:px-10 sm:py-32">
      ...
      <section id="precios" className="mx-auto max-w-[1240px] px-5 pt-24 sm:px-10 sm:pt-32">
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-[minmax(0,1.25fr)_minmax(0,.75fr)]">
          {PLANS.filter((p) => p.featured).map((plan) => (
            <div key={plan.key} className="grid content-start rounded-[26px] bg-[var(--void)] p-8 text-[var(--on-void)] sm:p-10">
              <div className="flex items-baseline justify-between gap-4">
                <h3 className="display-sm text-[22px]">{plan.name}</h3>
                <span className="mono text-[var(--lit-dawn)]">Más elegido</span>
              </div>
              <p className="mono mt-1.5 text-[var(--on-void-60)]">{plan.kicker}</p>
              <p className="display mt-6 text-[76px] leading-none tabular-nums">
                <span className="align-super text-[26px]">$</span>
                {plan.price}
                <span className="mono ml-3 align-middle text-[var(--on-void-60)]">{priceLabel(plan).unit}</span>
              </p>
              <p className="lede mt-5 max-w-[34ch] text-[var(--on-void-60)]">{plan.line}</p>
              ...
              <a href={registerUrl(plan.appPlan!)} className="allok-btn allok-btn-sky mt-9 w-full">
                Empezar con {plan.name}
              </a>
              {plan.trialDays ? (
                <p className="mono mt-3 text-center text-[var(--on-void-60)]">{plan.trialDays} días de prueba</p>
              ) : null}
            </div>
          ))}
          ...
          <div className="grid content-start gap-4">
            {PLANS.filter((p) => !p.featured).map((plan) => (
              <div key={plan.key} className="grid content-start rounded-[26px] border border-[var(--line)] bg-white p-7">
                <div className="flex items-baseline justify-between gap-4">
                  <h3 className="display-sm text-[18px]">{plan.name}</h3>
                  <p className="display text-[30px] leading-none tabular-nums">
                    {plan.from ? <span className="mono mr-1.5 align-middle text-[var(--ink-40)]">desde</span> : null}
                    <span className="align-super text-[15px]">$</span>
                    {plan.price}
                  </p>
                </div>
                <p className="mono mt-1.5 text-[var(--ink-40)]">
                  {plan.kicker} · {priceLabel(plan).unit}
                </p>
                <p className="mt-3 text-[14.5px] leading-snug text-[var(--ink-60)]">{plan.line}</p>
                <a
                  href={plan.appPlan ? registerUrl(plan.appPlan) : whatsappUrl(plan.talkTo ?? DEMO)}
                  className="allok-btn mt-6 w-full border border-[rgba(16,17,18,.22)] !py-3 !text-[15px] text-[var(--ink)]"
                >
      ...
      <section className="mx-auto max-w-[1240px] px-5 pt-24 sm:px-10 sm:pt-32">
